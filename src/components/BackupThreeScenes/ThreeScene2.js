...
import * as VectorCalculations from './services/VectorCalculations'
const ThreeScene = ({ markObjectiveComplete, levelNumber, loading, setLoading }) => {
  ...
  const calculateTiltAxis = (vector) => {
    return VectorCalculations.calculateTiltAxis(vector, droneBody.current);
  };

  const calculateYawCameraOffset = (vector) => {
    return VectorCalculations.calculateYawCameraOffset(vector, droneBody.current);
  };
  ...
  useEffect(() => {
    ...
    const updateMovement = () => {
      ...
      if(rotorsOn.current){
        ...
        if (droneBody.current) {
                ...
                const tiltAxis = calculateTiltAxis(new THREE.Vector3(tiltX, 0, -tiltZ).normalize()); // Adjust axis if needed
    ...
    const updateCamera = () => {
      if (droneBody.current && camera.current) {
        const offset = calculateYawCameraOffset(new THREE.Vector3(0, 2, 5)); // Adjust this offset as needed
